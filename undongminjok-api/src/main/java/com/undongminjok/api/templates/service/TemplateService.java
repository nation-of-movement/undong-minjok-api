...
public class TemplateService {
    ...
    Map<Integer, List<WorkoutPlanExerciseDTO>> grouped =
        exercises.stream()
                 .sorted(Comparator
                             ...
                             .thenComparing(WorkoutPlanExercise::getOrderIndex,
  ...
  public void updateTemplate(Long templateId, TemplateUpdateRequestDTO req) {
    Long loginUserId = SecurityUtil.getLoginUserInfo()
                                   .getUserId();
    ...
    Map<Long, WorkoutPlanExercise> currentMap = plan.getExercises()
                                                    .stream()
                                                    .filter(e -> e.getId() != null)
                                                    .collect(
                                                        Collectors.toMap(WorkoutPlanExercise::getId,
                                                                         e -> e));
  ...
  public void deleteTemplate(Long templateId) {
    ...
    Template template = templateRepository.findById(templateId)
                                          .orElseThrow(
                                              () -> new IllegalArgumentException("삭제할 템플릿 없음"));
    ...
    if (template.getSalesCount() >= 1) {
      templateStorageRepository.deleteByUserUserIdAndTemplateId(userId, templateId);
